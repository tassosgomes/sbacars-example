...
// SPA's spans from catalog-service's own — same word, different signal, on purpose (the browser
// and the backend service happen to share a domain name here, not an identity).
 ...
 * automatically and to the gateway's cross-origin one via `propagateTraceHeaderCorsUrls` — this is
 * the mechanism that makes the SPA the place a trace is *born*, not just where it is displayed.
 *
...
export function initTelemetry(): void {
  ...
  // Only the gateway this app actually calls is cross-origin from the SPA's own origin (5173 vs.
  // 5000) — propagateTraceHeaderCorsUrls is an allow-list, not a default-on: without it, the
...
/**
 * §5.7/§8: no personal data in a span attribute. The fetch/XHR instrumentations' default URL
 * attribute (`url.full`, `http.url` on older semantic conventions) includes the query string — a
 * stray `?email=...` or `?cpf=...` on a future endpoint would otherwise leak straight into the
 * exported span. Every URL this app talks to is stripped of its query string unconditionally,
 * rather than trying to recognize case by case which query string is "safe" — the same
 * last-mile-before-export shape as the backend's `SensitiveDataRedactionProcessor`
 * (`BaseProcessor<Activity>.OnEnd`), just on the two attribute names the JS SDK actually uses.
 */
