...
/**
 * Proves the one behavior A8 requires an automated test for on the frontend side: a call to the
 * gateway carries the W3C `traceparent` header (§8) — the mechanism that makes the trace born in
 * the browser continue into the backend at all. `initTelemetry` patches the global `fetch`, so the
 * mock is installed first and the module is imported fresh per test to get a clean
...
describe('OpenTelemetry Web — traceparent propagation', () => {
  ...
  it('attaches a traceparent header to a call made to the configured API origin', async () => {
    ...
    // http://localhost:5000 is gateway-public's default API_BASE_URL (see runtimeConfig's
    ...
    await fetch('http://localhost:5000/api/catalog/vehicles');
